...
class RefundMiddle extends \app\base\middle\BaseMiddle {
    ...
    protected function info() {
        ...
        if($orderInfo['order_seller_id']) {
            //卖家接口
            $merchantInfo = target('merchant/Merchant')->getInfo([
                'A.user_id' => $orderInfo['order_seller_id']
            ]);
            $receiveInfo = [
                'name' => $merchantInfo['contact_name'],
                'tel' => $merchantInfo['contact_tel'],
                'address' => $merchantInfo['contact_province'].$merchantInfo['contact_city'].$merchantInfo['contact_region'].$merchantInfo['contact_address'],
            ];
        }else {
