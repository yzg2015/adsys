...
class SiteFormHtmlModel {
    ...
    public function datetime($label, $must, $tip, $data, $config) {
        ...
        return '<input type="text" data-dux="form-date" data-format="yyyy-mm-dd hh:ii" name="' . $label . '" value="' . $data . '" placeholder="' . $tip . '" ' . $must . '>';
    }
    ...
    public function image($label, $must, $tip, $data, $config) {
        ...
        $html = '<div class="am-input-group">
                    ...
                    <span class="am-input-group-btn">
                        <button class="am-btn am-btn-default" type="button" data-dux="form-upload" data-target="#' . $label . '" data-type="jpg,png,bmp" ' . $resize . '><i class="am-icon-upload"></i></button>
                    </span>
                 </div>';
        return $html;
    }
    ...
    public function images($label, $must, $tip, $data, $config) {
        ...
        $resize = $width && $height ? ' data-resize="{width : ' . $width . ', height: ' . $height . ', crop: true}" ' : '';
        $html = '<button class="am-btn am-btn-default" type="button" data-dux="form-images" data-img-list=\'' . $images . '\' data-img-name="' . $label . '" data-img-warp="#dux-images-' . $label . '" data-type="jpg,png,bmp" ' . $resize . '><i class="am-icon-upload"></i> 上传组图</button>';
    ...
    public function file($label, $must, $tip, $data, $config) {
        ...
        $html = '<div class="am-input-group">
                    <input type="text" name="' . $label . '" id="' . $label . '" value="' . $data . '" placeholder="' . $tip . '" ' . $must . '>
                    <span class="am-input-group-btn">
                        <button class="am-btn am-btn-default" type="button" data-dux="form-upload" data-target="#' . $label . '" ' . $config . '><i class="am-icon-upload"></i></button>
                    </span>
                 </div>';
        return $html;
    }
    ...
    public function files($label, $must, $tip, $data, $config) {
        ...
        $html = '<button class="am-btn am-btn-default" type="button" data-dux="form-files" data-file-list=\'' . $files . '\' data-file-name="' . $label . '" data-file-warp="#dux-files-' . $label . '" data-type="'.$config.'"><i class="am-icon-upload"></i> 上传文件</button>';
    ...
    public function area($label, $must, $tip, $data, $config) {
        $must = $must ? 'required' : '';
        $area = explode(',', $data);
        $html = '<div class="dux-g" data-dux="form-location" ' . $must . '>
                    <div class="am-u-md-4">
    ...
    public function baidumap($label, $must, $tip, $data, $config) {
        ...
        $html = '<div class="am-input-group">
                    <input type="text" name="' . $label . '" id="' . $label . '" value="' . $data . '" placeholder="' . $tip . '" ' . $must . '>
                    <span class="am-input-group-btn">
                        <button class="am-btn am-btn-default" type="button" data-dux="form-map" data-id="' . $label . '"><i class="am-icon-upload"></i></button>
                    </span>
                 </div>';
        return $html;
    }
