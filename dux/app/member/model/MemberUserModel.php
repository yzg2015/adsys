...
class MemberUserModel extends SystemModel {
    ...
    public function getUser($uid) {
        ...
        $infoData = [
            ...
            'sex' => $info['sex'],
            'birthday' => $info['birthday'] ? date('Y-m-d', $info['birgetVerifythday']) : '',
        ...
        $infoData['show_name'] = $this->getNickname($info['nickname'], $info['tel'], $info['email']);
        ...
        $pointsInfo = target('member/PointsAccount')->getWhereInfo(['A.user_id' => $uid]);
        if (empty($pointsInfo)) {
            $this->error = '积分账户不存在!';
            return false;
        }
        $infoData['points_account_id'] = $pointsInfo['account_id'];
        $infoData['points'] = $pointsInfo['points'];
        $infoData['points_spend'] = $pointsInfo['spend'];
        $infoData['points_charge'] = $pointsInfo['charge'];
        return $infoData;
    }
