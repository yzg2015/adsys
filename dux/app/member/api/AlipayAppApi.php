...
class AlipayAppApi implements PayNotifyInterface {
    ...
    public function index() {
        dux_log('test');
        echo target('member/AlipayApp', 'pay')->notifyPay($this);
    }
    ...
    public function notifyProcess(array $data) {
        ...
        if(!target($callbackInfo['target'], 'service')->pay($orderNo, $data['amount'], '支付宝APP', $data['transaction_id'], 0, 'alipay_app')) {
            $model->rollBack();
