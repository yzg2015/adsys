...
class SiteMobile extends \app\base\controller\BaseController {
    ...
    public function __construct() {
        ...
        if(!$this->siteConfig['config_site_status']) {
            \Dux::errorPage('站点维护', $this->siteConfig['config_site_error']);
        }
    }
    ...
    protected function htmlPage($pageData, $params = []) {
        ...
        if (method_exists($this, '_pageHtml')) {
            ...
            $html = '<div class="prev"><a href="{prevUrl}">上一页</a></div><div class="number"><span class="cur">第{current}页</span> <i class="am-icon-angle-down"></i><select onchange="window.location.href = this.options[this.selectedIndex].value;">';
