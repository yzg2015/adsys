    ...
    protected function data() {
        ...
        if (empty($saleInfo) && empty($applyInfo)) {
            ...
            if ($config['apply_type'] == 2) {
                ...
                $html = '本店累计消费次数达到' . $applyWhere['data'] . '次，才可成为本店分销商，您已消费' . $count . '次，';
            ...
            if ($config['apply_type'] == 3) {
                ...
                $html = '本店消费金额满' . $applyWhere['data'] . '元，才可成为本店分销商，您已消费' . $count . '元，';
                if ($count < $applyWhere['data']) {
                    $html .= '请继续努力！';
                } else {
                    $html .= '可以立即申请！';
            ...
            if (!empty($saleInfo)) {
                $html = '恭喜您，您已经成功加入分销商，加入时间为【' . date('Y-m-d H:i', $saleInfo['agent_time']) . '】，您可以进行正常推广。';
            }else {
                switch ($applyInfo['status']) {
                    case 1:
                        $html = '您的分销商申请已提交成功，请耐心等待管理员的审核！';
                        break;
                    case 0:
                        $html = '很抱歉，您的分销商申请已被拒绝，拒绝原因【'.$applyInfo['remark'] ? $applyInfo['remark'] : '无'.'】';
                        break;
                }
            }
        }
